...
type TabLabel = typeof dataArray[number]['label']

export function Connections() {
  const [currentTab, setCurrentTab] = useState<TabLabel>("Followers");
        ...
        <div className="inline-flex flex-wrap items-center border border-gray-200 rounded-md p-1 bg-white shadow-sm">
          {
            dataArray.map((tab) => (
              <button
                ...
                className={`
                  flex items-center px-3 py-1 text-sm rounded-md transition-colors cursor-pointer
        ...
        <div className="flex flex-wrap gap-6 mt-6">
          {
            dataArray.find(item => item.label === currentTab)?.value.map(user => (
              <div
                key={user._id}
                className="w-full max-w-96 flex gap-5 p-6 bg-white shadow rounded-md"
              >
                <img
                ...
                <div className="flex-1 flex flex-col justify-between">
                  ...
                  <div className="flex max-sm:flex gap-2 mt-4">
                    <Link
                      to={`/profile/${user._id}`}
                      className="w-full p-2 text-sm text-center rounded bg-linear-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 active:scale-95 transition text-white cursor-pointer"
                    >
                      View Profile
                    </Link>
